...
function frag = compile(frag)
    ...
    while 1,
        ...
        if isempty(TagStart)
            ...
            temp_count=Xparse_count+1;
            xtree{temp_count} = chardata;
...
function frag = tag_element(frag)
    ...
    if isempty(close)
        ...
        temp_count=Xparse_count+1;
        xtree{temp_count} = element;
        xtree{temp_count}.name = strip(name);
