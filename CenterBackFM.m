function [ center_ver ] = CenterBackFM( atlas_parcel, neighbor, max_neighbor, vertice_loc)
...
%find edge vertex
for num=1:N_parcel
    ind=find(atlas_parcel==num);
    for j=1:length(ind)
        cur=neighbor(ind(j),:);
        for k=1:max_neighbor(ind(j))
            if atlas_parcel(cur(k))~=num
                ver_label(ind(j))=1;
                break;
            end
        end
